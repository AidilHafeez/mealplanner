...
// variables for read the data from their id
...
btnUpdate.addEventListener('click', function(){  //updating text file when user click UPDATE button
    let file = path.join(pathName, fileName.value)
    let contents = fileContents.value
    fs.appendFile(file, contents, function(err){ //param1: textfile yg kita nak update param2: apa yg kita nak update ke text file
        if(err) throw err;
